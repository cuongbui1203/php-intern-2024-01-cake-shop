...
export default function FilterCake({onChangeData=()=>{}}) {
  ...
  return (
    <div className='flex flex-col'>
      ...
      <Checkbox.Group options={cakeType} onChange={onChange} value={checkedList}/>
      </div>
      <div>
      <Label value={'Price'} />
      <Radio.Group options={priceOpt} onChange={onChange1} value={value1}  />
       </div>
      <div className='flex justify-center w-full'>
        <button onClick={handle} className='px-4 py-2 ml-3 font-bold text-white bg-blue-500 rounded hover:bg-blue-700'>ok</button>
      </div>
    </div>
  )
}
